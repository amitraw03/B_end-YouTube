// basically we r using controllers as a separate extension {on routing certain endpoints the functions work here} which makes it industry std
...
const registerUser = asyncHandler(async (req, res) => {
   ...
   //(5) upload them to cloudinry & avatar checking after uloading
   ...
   // const hashPassword = await bcrypt.hash(password,10)  //manually doing work of hashing pass
   ...
   //(7) remove password & refresh token field while sending response
...
const loginUser = asyncHandler(async (req, res) => {
   ...
   // data we want to send now to loginUser after all above 4 steps 
...
const logoutUser = asyncHandler(async (req, res) => {
   //now becoz of auth middleware i have access of user credentials in ' req '
...
//Refreshing Your Access Token using session storage(i.e refresh token  stored in d.b too)
...
const getCurrentUser = asyncHandler(async (req, res) => {
   ...
   return res.status(200)
      .json({
         status: 200,     // here we r not using ApiResponse so .json() method should receive an object, not multiple arguments
         user: req.user,
